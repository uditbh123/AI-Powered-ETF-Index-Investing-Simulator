...
def get_news_feed(
    ...
    is the mean of non-null sentiment scores in the window (null when there are
    no scored headlines). Headlines are hard-capped at 100 rows.
    """
    ...
    if category == "sector" and ticker_id is not None:
        rows = news_dao.list_sentiment(
            conn, category=category, ticker_id=ticker_id, start=start, desc=True
        )
    else:
        rows = news_dao.list_sentiment(conn, category=category, start=start, desc=True)

    headlines = rows[:MAX_HEADLINES]
